...
{strip}
	...
	{assign var=MODULE value='ModuleBuilder'}
						...
						Relate the module under existing modules
						</br>
						<table class="table table-bordered blockContainer showInlineTable relatedlists2">
								<tr>
									<th class="module-title">&nbsp;</th>
									<th class="module-title">{vtranslate('MODULE_NAME', $MODULE)}</th>
									<th class="module-title">
								...
								{foreach from=$RELATED_LIST key=m item=v}
								{assign var=moduletks value=$m|@getTranslatedString:'$MODULE'}
								<tr class="relblock">
									<td><input type="checkbox" tabindex="" value="{$m}" id="{$j}" class="relcheck2" name="{$m}"/></td>
									<td>
										{$moduletks}
										{if $m eq 'Leads'}
										&nbsp;&nbsp;
									...
									<td><input type="checkbox" tabindex="" name="{$m}_sel2" disabled="disabled" class="small relsel2"></td>
									<td><input type="checkbox" tabindex="" name="{$m}_add2"  disabled="disabled" class="small reladd2"></td>
								</tr>

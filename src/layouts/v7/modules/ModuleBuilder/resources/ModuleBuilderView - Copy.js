...
jQuery.Class("ModuleBuilder_ModuleBuilderView_Js",{
			...
			app.helper.showConfirmationBox({'message' : message}).then(
				function(e) {
                   ...
                   builddata['tabid']= jQuery('#tabid').val();
					...
					var checkedItems2 =  jQuery('.relatedlists2').find('input[type="checkbox"]:checked');//...
					...
					jQuery.each(checkedItems2,function(index,domElement){	
						var checkeditems2 	= jQuery(domElement);
						builddata[checkeditems2.attr('name')] = checkeditems2.val();
					});
 ...
 emptyBlockValidation : function() {
	...
	registerCMRelatedListEvent : function(){
        var thisInstance = this;
		contents 		 = jQuery('#layoutEditorContainer').find('.relatedListTab');
		relatedContainer 		 = contents.find('.relblock');
		relatedContainer.find('.relcheck2').change(function(e) {
												 
			var currentTarget 	= jQuery(e.currentTarget);
			var row 			= currentTarget.closest('tr');
			var selectcheck 	= row.find('.relsel2');
			var addcheck 		= row.find('.reladd2');
			var status 			= jQuery(e.currentTarget).is(':checked');
			if(status){
				selectcheck.removeAttr( "disabled" );
				addcheck.removeAttr( "disabled" );
   ...
   */
   ...
   if(!returnBool) return false;  
	...
	registerEvents : function() {
		...
		thisInstance.registerCMRelatedListEvent();
